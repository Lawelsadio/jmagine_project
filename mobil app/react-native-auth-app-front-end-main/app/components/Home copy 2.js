...
function Home() {
  const { setIsLoggedIn, profile,role, setRole } = useLogin();
  const [parcours, setParcours] = useState( [] );
  const [fullparcour, setfullParcour] = useState( [] );


  useEffect(() => {
    axios.get('http://10.192.27.79:8886/api/parcours').then((response) => {
   ...
   /*const flullParcour = () => {
var parcourId = parcour.id
    axios.get(`http://10.192.27.79:8886/api/parcours/${parcourId}/full`).then((response) => {
      console.log("response.data", response.data)
      setfullParcours(response.data);
    });
    console.log(fullparcours)
    return fullparcours;
  };
